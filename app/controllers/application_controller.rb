class ApplicationController < ActionController::Base
  ...
  def failed_two_factor_auth_key
    "zimbitx:session:#{request.ip}:failed_two_factor_auths"
  end
  ...
  def save_session_key(member_id, key)
    Rails.cache.write "zimbitx:sessions:#{member_id}:#{key}", 1, expire_after: ENV['SESSION_EXPIRE'].to_i.minutes
  end

  def clear_all_sessions(member_id)
    if redis = Rails.cache.instance_variable_get(:@data)
      redis.keys("zimbitx:sessions:#{member_id}:*").each {|k| Rails.cache.delete k.split(':').last }
    end

    Rails.cache.delete_matched "zimbitx:sessions:#{member_id}:*"
  end

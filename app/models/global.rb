class Global
  ...
  class << self
    ...
    def daemon_statuses
      Rails.cache.fetch('zimbitx:daemons:statuses', expires_in: 3.minute) do
  ...
  def key(key, interval=5)
    ...
    "zimbitx:#{@currency}:#{key}:#{time_key}"
  end

  def asks
    Rails.cache.read("zimbitx:#{currency}:depth:asks") || []
  end

  def bids
    Rails.cache.read("zimbitx:#{currency}:depth:bids") || []
  end
  ...
  def ticker
    ticker           = Rails.cache.read("zimbitx:#{currency}:ticker") || default_ticker
    open = Rails.cache.read("zimbitx:#{currency}:ticker:open") || ticker[:last]
  ...
  def trades
    Rails.cache.read("zimbitx:#{currency}:trades") || []
  end

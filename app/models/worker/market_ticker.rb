module Worker
  class MarketTicker
    ...
    def update_ticker(trade)
      ...
      Rails.cache.write "zimbitx:#{trade.market.id}:ticker", ticker
    end
    ...
    def update_latest_trades(trade)
      ...
      Rails.cache.write "zimbitx:#{trade.market.id}:trades", trades
    end
    ...
    def initialize_market_data(market)
      ...
      Rails.cache.write "zimbitx:#{market.id}:trades", @trades[market.id]
      ...
      Rails.cache.write "zimbitx:#{market.id}:ticker", @tickers[market.id]
    end

    private

    def get_market_low(market, trade)
      low_key = "zimbitx:#{market}:h24:low"
    ...
    def get_market_high(market, trade)
      high_key = "zimbitx:#{market}:h24:high"
    ...
    def initialize_market_low(market)
      if low_trade = Trade.with_currency(market).h24.order('price asc').first
        ...
        write_h24_key "zimbitx:#{market}:h24:low", low_trade.price, ttl
        low_trade
      end
    end
    ...
    def initialize_market_high(market)
      if high_trade = Trade.with_currency(market).h24.order('price desc').first
        ...
        write_h24_key "zimbitx:#{market}:h24:high", high_trade.price, ttl
        high_trade
      end
    end
